...
description: >-
  ...
  (https://github.com/npryce/adr-tools). Use when the user asks to "add an
  ADR", "record this decision", "create an ADR", "supersede ADR N", "link
  ADRs", "generate the ADR table of contents", or any request involving the
  `adr` command. Pairs with the adr-author agent; this skill is the mechanics
  (commands, files, links), and adr-author is the decision framework and prose
  authoring.
role: worker
user-invocable: true
---
